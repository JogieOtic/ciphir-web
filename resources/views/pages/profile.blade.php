...
<body style="
    ...
    @if (session('success'))
        <div id="alert-success" class="z-50 absolute top-1/3 left-1/2 -translate-x-1/2 bg-blue-500 text-white p-4 rounded-lg">
            {{ session('success') }}
        </div>
    @endif
    {{-- notify the actions --}}
    @if (session('error'))
        <div id="alert-error" class="z-50 absolute top-1/3 left-1/2 -translate-x-1/2 bg-red-500 text-white p-4 rounded-lg">
            {{ session('error') }}
        </div>
    @endif
    ...
    <div class="w-screen h-screen grid backdrop-blur-sm place-items-center bg-transparent">
        <div class="w-full max-w-sm p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8  ">
            <form class="space-y-6" action="{{route('change-username')}}" method="POST">
                ...
                <div>
                    <label for="username" class="block mb-2 text-sm font-medium text-gray-900 ">Usermame</label>
